...
function monthFrom(nl){
  ...
  for (const k of Object.keys(MONTHS)) if (key.startsWith(k)) return MONTHS[k];
  return null;
}
function dedupe(arr){
...
function parseAriaLabels(labels){
  ...
  for (const a of labels) {
    ...
    if (!m) continue;
    const d = +m[2];
    const mo = monthFrom(m[3]); if (!mo) continue;
    const y = +m[4];
    ...
    if (!isNaN(+start) && !isNaN(+end)) events.push({ start, end, title: title || 'Les', location });
  }
...
(async () => {
  ...
  if (await passField.count()) { await passField.fill(password); await passField.press('Enter'); }
